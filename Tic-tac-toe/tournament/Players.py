    ...
    def __init__(self, is_black):
        ...
        if is_black:
            self.model.load_state_dict(
                torch.load("model11_black_{}.pth".format(64), map_location=lambda storage, loc: storage)
            )
        else:
            self.model.load_state_dict(
                torch.load("model11_white_{}.pth".format(64), map_location=lambda storage, loc: storage)
            )
        self.model.eval()
